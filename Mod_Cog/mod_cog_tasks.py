        ...
        if isinstance(period, str):
            for t in range(int(self.timing["decision"] / self.dt)):
                self.gt[self.start_ind["decision"] + t] = np.mod(
                    value + int(seq * t * self.omega * self.dt), self.dim_ring
                )
...
class _Reach(EnvWithAdditions):
    ...
    def __init__(
        ...
        self.timing = {"fixation": 500, "stimulus": 500, "delay": 500, "decision": 500}
        if not dly:
    ...
    def _new_trial(self, **kwargs):
        ...
        stim_theta = np.mod(
            self.theta[ground_truth]
            + int(trial["integrate"]) * self.omega * self.timing["delay"]
            + int(trial["anti"]) * np.pi,
            2 * np.pi,
        )
        ...
        self.my_set_groundtruth(
            value=ground_truth, period="decision", where="choice", seq=self.seq
        )
...
class _DMFamily(EnvWithAdditions):
    ...
    def _new_trial(self, **kwargs):
        ...
        if self.integrate == 1:
            i_target = int(
                np.mod(i_target + self.omega * self.timing["delay"], self.dim_ring)
            )
        if self.integrate == -1:
            i_target = int(
                np.mod(i_target - self.omega * self.timing["delay"], self.dim_ring)
        ...
        self.my_set_groundtruth(
            i_target, period="decision", where="choice", seq=self.seq
        )
...
class _DelayMatch1DResponse(EnvWithAdditions):
    ...
    def _new_trial(self, **kwargs):
        ...
        if self.integrate == 1:
            i_test_theta = int(
                np.mod(i_test_theta + self.omega * self.timing["delay"], self.dim_ring)
            )
        if self.integrate == -1:
            i_test_theta = int(
                np.mod(i_test_theta - self.omega * self.timing["delay"], self.dim_ring)
        ...
        if (ground_truth == "match" and self.matchgo) or (
            ...
            self.my_set_groundtruth(
                i_test_theta, period="decision", where="choice", seq=self.seq
            )
        else:
...
def dmsseqr(**kwargs):
    return _dlymatch(
        matchto="sample", matchgo=True, seq=1, timing={"decision": 1000}, **kwargs
    )
...
def dnmsseqr(**kwargs):
    return _dlymatch(
        matchto="sample", matchgo=False, seq=1, timing={"decision": 1000}, **kwargs
    )
...
def dmcseqr(**kwargs):
    return _dlymatch(
        matchto="category", matchgo=True, seq=1, timing={"decision": 1000}, **kwargs
    )
...
def dnmcseqr(**kwargs):
    return _dlymatch(
        matchto="category", matchgo=False, seq=1, timing={"decision": 1000}, **kwargs
    )
