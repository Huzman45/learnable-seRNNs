# Based on task.py from the multitask repo by @gyyang https://github.com/gyyang/multitask
...
class Trial(object):
    ...
    In TensorFlow 2.x, NumPy arrays will be automatically converted to TensorFlow
    tensors when passed to a tf.keras.Model. Therefore, this class can continue
    to store data as NumPy arrays.
    """
    ...
    def add(self, loc_type, locs=None, ons=None, offs=None, strengths=1, mods=None):
        ...
        for i in range(self.batch_size):
            if loc_type == "fix_in":
                ...
                if self.config["loss_type"] == "lsq":
                    self.y[ons[i] : offs[i], i, 1:] += (
                        self.add_y_loc(locs[i]) * strengths[i]
                    )
                else:
    ...
    def add_x_noise(self):
        """Add input noise."""
        # This remains a NumPy operation, which is fine
...
def _contextdm(config, mode, attend_mod, **kwargs):
    ...
    if mode == "random":  # Randomly generate parameters
        ...
        if (attend_mod == 1) or (attend_mod == 2):
            stim1_mod1_strengths, stim2_mod1_strengths = contextdm_genstim(
                batch_size, rng, stim_coh_range
            )
            stim1_mod2_strengths, stim2_mod2_strengths = contextdm_genstim(
                batch_size, rng, stim_coh_range
            )
            if attend_mod == 1:
                stim1_strengths, stim2_strengths = (
                    stim1_mod1_strengths,
                    stim2_mod1_strengths,
                )
            else:
                stim1_strengths, stim2_strengths = (
                    stim1_mod2_strengths,
                    stim2_mod2_strengths,
                )
        else:
            stim1_strengths, stim2_strengths = contextdm_genstim(
                batch_size, rng, stim_coh_range
            )
...
def _dm(config, mode, stim_mod, **kwargs):
    ...
    if mode == "random":  # Randomly generate parameters
        ...
        batch_size = np.prod(batch_shape)
        ind_stim_loc, ind_stim1_strength = np.unravel_index(
            range(batch_size), batch_shape
        )
        fix_offs = int(2000 / dt)
...
def dms_(config, mode, matchnogo, **kwargs):
    ...
    if mode == "random":  # Randomly generate parameters
        ...
        stim_dist = rng.uniform(
            np.pi / 9, np.pi * 17.0 / 9.0, (batch_size,)
        ...
        batch_size = np.prod(batch_shape)
        ind_stim_loc, ind_mod1, ind_mod2 = np.unravel_index(
            range(batch_size), batch_shape
        )
        ...
        stim1_locs = p["stim1_locs"]
        stim2_locs = p["stim2_locs"]
        matchs = get_dist(stim1_locs - stim2_locs) < np.pi / 36.0  # 5 degree
        batch_size = len(stim1_locs)
...
def dmc_(config, mode, matchnogo, **kwargs):
    ...
    if mode == "random":  # Randomly generate parameters
        ...
        stim1_locs = rng.choice(
            np.array([0.1, 0.3, 0.5, 0.7, 0.9, 1.1, 1.3, 1.5, 1.7, 1.9]) * np.pi,
            size=(batch_size,),
        )
        ...
        stim1_locs_ = (
            np.concatenate(
                (
                    ...
                    (1.1 + 0.8 * np.arange(n_stim_loc2) / n_stim_loc2),
                )
            )
            * np.pi
        )
...
def oic(config, mode, **kwargs):
    ...
    if mode == "random":  # Randomly generate parameters
        ...
        stim1_locs = (
            np.concatenate(
                ((0.1 + 0.8 * np.arange(a) / a), (1.1 + 0.8 * np.arange(a) / a))
            )
            * np.pi
        )
